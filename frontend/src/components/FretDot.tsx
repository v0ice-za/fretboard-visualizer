...
const DOT_CONFIG: Record<DotState, { radius: number; fill: string; opacity: number }> = {
  root:     { radius: 13, fill: '#f59e0b', opacity: 1 },
  scale:    { radius: 11, fill: '#6366f1', opacity: 0.85 },
  mode:     { radius: 11, fill: '#fb7185', opacity: 0.9 },
  freeform: { radius: 11, fill: '#22d3ee', opacity: 0.8 },
};
...
export default function FretDot({ fret, string: stringIdx, state, cx, cy }: FretDotProps) {
  const { radius, fill, opacity } = DOT_CONFIG[state];
